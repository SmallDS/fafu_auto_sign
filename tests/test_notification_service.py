...
services_pkg.__path__ = [
    os.path.join(os.path.dirname(__file__), "..", "src", "fafu_auto_sign", "services")
]
...
class TestNotifyBasicFunctionality:
    ...
    def test_notify_with_task_id_and_success(self, notification_service):
        ...
        result = notification_service.notify(
            "签到完成", "任务已执行", task_id="task_123", success=True
        )
        assert result is True
...
class TestDeduplicationLogic:
    ...
    def test_duplicate_within_5min_blocked(self, mock_sc_send, notification_service):
        """测试 5 分钟内重复通知被去重。"""
        # 第一次发送
        result1 = notification_service.notify("第一次", "内容", task_id="task_1", success=True)
        assert result1 is True

        # 相同任务和状态的第二次发送应该被去重
        result2 = notification_service.notify("第二次", "内容", task_id="task_1", success=True)
    ...
    def test_same_task_different_success_not_blocked(self, mock_sc_send, notification_service):
        ...
        # 第一次发送（成功）
        result1 = notification_service.notify("成功通知", "内容", task_id="task_1", success=True)
        assert result1 is True

        # 相同任务但失败状态的第二次发送应该允许
        result2 = notification_service.notify("失败通知", "内容", task_id="task_1", success=False)
    ...
    def test_different_task_id_not_blocked(self, mock_sc_send, notification_service):
        ...
        # 第一个任务
        result1 = notification_service.notify("任务1", "内容", task_id="task_1", success=True)
        assert result1 is True

        # 第二个任务的通知应该允许
        result2 = notification_service.notify("任务2", "内容", task_id="task_2", success=True)
        assert result2 is True
...
class TestSendNotification:
    ...
    def test_send_success_logs_info(self, mock_sc_send, notification_service, caplog):
        ...
        with caplog.at_level(logging.INFO):
            notification_service._send_notification("SCT12345", "测试标题", "测试内容")
